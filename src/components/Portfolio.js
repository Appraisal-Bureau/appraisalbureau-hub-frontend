...
function Portfolio() {
    ...
    const portfolioDataWithColorKey = portfolio.map((item, index) => ({
        color: <div className="color-key" style={{backgroundColor: barColors[index % 4]}} />, 
        name: item.name, 
        value: item.value
    }));
    ...
    return (
        <>
            ...
            <div className="big-number">{currencyFormatter.format(calculatePortfolioTotal(portfolio))}</div>
            <PortfolioVisualization data={portfolio} barColors={barColors} />
            <MuiTable columns={[{key: 'color', width: "30px"}, {key: 'name'}, {key: 'value'}]} data={portfolioDataWithColorKey} hideHeader={true}/>
        </>
    )
};
